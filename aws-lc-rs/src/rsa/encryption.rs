...
use aws_lc::{
    ...
    EVP_sha1, EVP_sha256, EVP_sha384, EVP_sha512, EVP_MD, EVP_PKEY, EVP_PKEY_CTX,
    RSA_PKCS1_OAEP_PADDING,
};
use core::{fmt::Debug, ptr::null_mut};

use super::key::{
    generate_rsa_key, rsa_key_size_enum, RsaEvpPkey, UsageContext, PKCS8_FIXED_CAPACITY_BUFFER,
};
...
impl AsDer<Pkcs8V1Der<'static>> for PrivateDecryptingKey {
    fn as_der(&self) -> Result<Pkcs8V1Der<'static>, Unspecified> {
        ...
        let out_len = {
            ...
            if 1 != unsafe { EVP_marshal_private_key(cbb.as_mut_ptr(), *self.0.key.as_const()) } {
                return Err(Unspecified);
            }
            cbb.finish()?
        };
...
impl AsDer<RsaPublicKeyX509Der<'static>> for PublicEncryptingKey {
    ...
    fn as_der(&self) -> Result<RsaPublicKeyX509Der<'static>, Unspecified> {
        ...
        if 1 != unsafe { EVP_marshal_public_key(der.as_mut_ptr(), *self.0.key) } {
            return Err(Unspecified);
        };
...
mod tests {
    use crate::{
        ...
        rsa::{
            encryption::{
                ...
                OAEP_SHA512_MGF1SHA512, EncryptionKeySize
            },
    ...
    #[test]
    fn generate() {
        let private_key = PrivateDecryptingKey::generate(EncryptionKeySize::Rsa2048).expect("generation");
